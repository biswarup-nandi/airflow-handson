...
SNOWFLAKE_DYNAMO_DATA_TABLE = 'DYNAMO_DATA_TBL'
SNOWFLAKE_FILE_FORMAT = 'JSON_FORMAT'
SNOWFLAKE_ROLE = 'ACCOUNTADMIN'
S3_FILE_PATH = 's3://testbkt120792/dynamodb-data/json/'
...
SNOWSQL_LOAD_DYNAMO_DATA_TABLE_STATEMENT = f'''MERGE INTO {SNOWFLAKE_DYNAMO_DATA_TABLE} AS target
                                                USING (
                                                    SELECT 
...
with DAG(
    dag_id='dynamo_s3_to_snowflake_v_1.0.1',
    default_args=default_args,
    description='Airflow DAG to Transfer Data From Dynamo DB data in S3 to Snowflake',
    ...
    SNOWSQL_LOAD_DYNAMO_DATA_TABLE_STATEMENT_TASK = SnowflakeOperator(
        task_id="SNOWSQL_LOAD_DYNAMO_DATA_TABLE_STATEMENT_TASK",
        snowflake_conn_id=SNOWFLAKE_CONN_ID,
        database=SNOWFLAKE_DATABASE,
        warehouse=SNOWFLAKE_WAREHOUSE,
        schema=SNOWFLAKE_SCHEMA,
        sql=SNOWSQL_LOAD_DYNAMO_DATA_TABLE_STATEMENT,
        role=SNOWFLAKE_ROLE
    )
    ...
    SNOWSQL_DROP_STAGE_STATEMENT_TASK >> SNOWSQL_CREATE_STAGE_STATEMENT_TASK >> SNOWSQL_COPY_STAGE_STATEMENT_TASK >> SNOWSQL_LOAD_DYNAMO_DATA_TABLE_STATEMENT_TASK >> SNOWSQL_REMOVE_DATA_FROM_STAGE_STATEMENT_TASK >> SNOWSQL_REMOVE_DATA_FROM_STAGE_TABLE_STATEMENT_TASK
